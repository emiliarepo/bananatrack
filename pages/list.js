...
export default function List({ data, length }) {
	...
	const getMore = async () => {
		const newStores = await (
			await fetch("api/prices?offset=" + stores.length)
		).json();
				...
				<section className="text-gray-600 body-font mb-6 min-h-screen">
					<div className="container px-5 py-4 mx-auto">
						<div className="flex flex-wrap -m-4 content-start">
							<Link href="/">
								<a className="p-4 mx-auto text-xl text-yellow-500 font-semibold">
									← Etusivulle
								</a>
							</Link>
